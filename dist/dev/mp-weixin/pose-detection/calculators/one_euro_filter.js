...
const poseDetection_calculators_constants = require("./constants.js");
const poseDetection_calculators_low_pass_filter = require("./low_pass_filter.js");
/**
 * @license
...
class OneEuroFilter {
  constructor(config) {
    ...
    this.x = new poseDetection_calculators_low_pass_filter.LowPassFilter(this.getAlpha(this.minCutOff));
    this.dx = new poseDetection_calculators_low_pass_filter.LowPassFilter(this.getAlpha(this.derivateCutOff));
  ...
  apply(value, microSeconds, valueScale) {
    ...
    if (this.lastTimestamp !== 0 && $microSeconds !== 0) {
      this.frequency = 1 / (($microSeconds - this.lastTimestamp) * poseDetection_calculators_constants.MICRO_SECONDS_TO_SECOND);
    }
