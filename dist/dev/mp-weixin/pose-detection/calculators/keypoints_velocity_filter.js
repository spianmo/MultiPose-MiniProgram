...
const poseDetection_calculators_relative_velocity_filter = require("./relative_velocity_filter.js");
...
class KeypointsVelocityFilter {
  ...
  initializeFiltersIfEmpty(keypoints) {
    if (this.xFilters == null || this.xFilters.length !== keypoints.length) {
      this.xFilters = keypoints.map((_) => new poseDetection_calculators_relative_velocity_filter.RelativeVelocityFilter(this.config));
      this.yFilters = keypoints.map((_) => new poseDetection_calculators_relative_velocity_filter.RelativeVelocityFilter(this.config));
      this.zFilters = keypoints.map((_) => new poseDetection_calculators_relative_velocity_filter.RelativeVelocityFilter(this.config));
    }
  }
}
