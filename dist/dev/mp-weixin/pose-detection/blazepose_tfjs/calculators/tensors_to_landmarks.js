"use strict";
const poseDetection_calculators_sigmoid = require("../../calculators/sigmoid.js");
/**
 * @license
 ...
 * =============================================================================
 */
function tensorsToLandmarks(landmarkTensor, config, flipHorizontally = false, flipVertically = false) {
  const numValues = landmarkTensor.size;
  const numDimensions = numValues / config.numLandmarks;
  const rawLandmarks = landmarkTensor.dataSync();
  ...
  for (let ld = 0; ld < config.numLandmarks; ++ld) {
    ...
    if (numDimensions > 3) {
      landmark.score = poseDetection_calculators_sigmoid.sigmoid(rawLandmarks[offset + 3]);
    }
