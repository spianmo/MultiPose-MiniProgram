...
const poseDetection_shared_filters_one_euro_filter = require("./one_euro_filter.js");
/**
 * @license
...
class KeypointsOneEuroFilter {
  ...
  initializeFiltersIfEmpty(keypoints) {
    if (this.xFilters == null || this.xFilters.length !== keypoints.length) {
      this.xFilters = keypoints.map((_) => new poseDetection_shared_filters_one_euro_filter.OneEuroFilter(this.config));
      this.yFilters = keypoints.map((_) => new poseDetection_shared_filters_one_euro_filter.OneEuroFilter(this.config));
      this.zFilters = keypoints.map((_) => new poseDetection_shared_filters_one_euro_filter.OneEuroFilter(this.config));
    }
  }
}
